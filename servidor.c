...
int kk=0;//variavel que permite controlar o numero de users ate chegar a 20-->usado no comando "users"
...
int t=0;
    ...
    if(strcmp(w.comando,"exit")==0){
        ...
        //mas dps fazer realloc de uma posicao, para continuar a ter sempre as 16 possibilidades de users a correr
        v_din=realloc(v_din,20*sizeof(user));

        for(i=0;i<20;i++){
            if(v_din[i].c.pids!=10){
                t++;
            }
        }
        printf("\n%d\n",t);
...
printf("\nLogados:\n");
    for(i=0;i<20;i++){//definir tamanho de jogadores
...
v_din = (userr) malloc(sizeof(user)*NUM_JOG);
